...
if [[ ! -f custom_mongo.properties ]]; then
        ...
        echo "url=jdbc:informix-sqli://${HOSTNAME}:9088/${DB_NAME:=sysmaster}:INFORMIXSERVER=${INFORMIXSERVER};USER=${DB_USER:=informix};PASSWORD=
${DB_PASS:=in4mix}" >> json_mongo.properties
...
if [[ ! -f custom_rest.properties ]]; then
        ...
        echo "url=jdbc:informix-sqli://${HOSTNAME}:9088/${DB_NAME:=sysmaster}:INFORMIXSERVER=${INFORMIXSERVER};USER=${DB_USER:=informix};PASSWORD=
${DB_PASS:=in4mix}"  >> json_rest.properties
...
if [[ ! -f custom_mqtt.properties ]]; then
        ...
        echo "url=jdbc:informix-sqli://${HOSTNAME}:9088/${DB_NAME:=sysmaster}:INFORMIXSERVER=${INFORMIXSERVER};USER=${DB_USER:=informix};PASSWORD=
${DB_PASS:=in4mix}"  >> json_mqtt.properties
