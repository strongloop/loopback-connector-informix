  ...
  describe.skip('Discover database schemas', function() {
    it('should return an array of db schemas', function(done) {
      db.connector.discoverDatabaseSchemas(function(err, schemas) {
        ...
        schemas.should.be.an.array; // eslint-disable-line no-unused-expressions
    ...
    function(done) {
      db.discoverSchema('INVENTORY', {owner: config.schema},
        function(err, schema) {
          ...
          assert(schema.options.informix.schema === config.schema);
          assert(schema.options.informix.table === 'INVENTORY');
          ...
          assert(schema.properties.productId.informix.columnName ===
            'PRODUCT_ID');
          assert(schema.properties.locationId);
          assert(schema.properties.locationId.type === 'String');
          assert(schema.properties.locationId.informix.columnName ===
            'LOCATION_ID');
