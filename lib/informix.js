...
Informix.prototype.destroyAll = function(
  ...
  var selectStmt = new ParameterizedSQL('SELECT COUNT(' + id + ') AS id');
  selectStmt += ' FROM ' + tableName;

  var executeTransaction = function(connection, cb) {
    debug('Informix.prototype.destroyAll %j %j', deleteStmt, selectStmt);
    ...
    connection.query(selectStmt, function(err, selectData) {
      ...
      connection.query(deleteStmt, null, function(err, deleteData) {
        ...
        return cb(null, selectData[0][id]);
      });
    });
  };
...
Informix.prototype.create = function(model, data, options, callback) {
  ...
  var executeTransaction = function(connection, cb) {
    debug('Informix.prototype.create %j %j', insertStmt, selectStmt);
    connection.query(insertStmt, function(err, insertData) {
      debug('Informix.prototype.create %j %j', insertStmt, insertData);
      if (err) {
        self.rollback(connection, function(err) {});
        return cb(err);
      }

      connection.query(selectStmt, function(err, selectData) {
        debug('Informix.prototype.create %j %j', selectStmt, selectData);
        if (err) {
          self.rollback(connection, function(err) {});
          return cb(err);
        }

        self.commit(connection, function(err) {});
        return cb(null, selectData[0][id]);
      });
    });
  };
  ...
  // occurs in the code below.
  if (options.transaction) {
    executeTransaction(options.transaction.connection, function(err, data) {
      if (err) {
        return callback(err);
      }
      callback(null, data);
...
Informix.prototype.update = function(
  ...
  var selectStmt = new ParameterizedSQL('SELECT COUNT(' + id + ') as id');
  selectStmt.sql += ' FROM ' + tableName;
  var updateData;

  var executeTransaction = function(connection, cb) {
    debug('Informix.prototype.update %j %j', updateStmt, selectStmt);
    var selectData;
    connection.query(updateStmt.sql, updateStmt.params, function(err, data) {
      if (err) {
        self.rollback(connection, function(err) {});
        return cb(err);
      }
