 ...
 */
Informix.prototype.destroyAll = function(model, where, options, callback) {
  ...
  var selectStmt = new ParameterizedSQL('SELECT ' + id + ' FROM ' + tableName);
  ...
  var executeTransaction = function(connection, cb) {
    connection.query(selectStmt, function(err, selectData) {
      debug(
        'Informix.prototype.destroyAll stmt: %j data: %j',
        selectStmt,
        selectData
      );
      if (err) {
...
Informix.prototype.create = function(model, data, options, callback) {
  ...
  var executeTransaction = function(connection, cb) {
    connection.query(insertStmt, options, function(err, data) {
      debug(
        'Informix.prototype.create stmt: %j data: %j err: %j',
        insertStmt,
        data,
        err
      );
      if (err) {
        ...
        connection.query(
          "SELECT DBINFO ('sqlca.sqlerrd1') as id " +
            'FROM systables ' +
            'WHERE tabid = 1',
          function(err, info) {
            debug('Informix.prototype.create info: %j err: %j', info, err);
            if (err) {
              return cb(err);
            } else {
              return cb(err, info[0].id);
            }
          }
        );
      }
 ...
 * @param {Function} callback The callback function
 */
Informix.prototype.update = function(model, where, data, options, callback) {
  var self = this;
  var tableName = self.tableEscaped(model);
  var id = self.idName(model);
  ...
  var executeTransaction = function(connection, cb) {
    connection.query(selectStmt.sql, selectStmt.params, function(err, data) {
      ...
      connection.query(updateStmt.sql, updateStmt.params, function(
        err,
        updateData
      ) {
        debug(
          'Informix.prototype.update stmt: %j data: %j',
          updateStmt,
          updateData
        );
        if (err) {
...
Informix.prototype.count = function(model, where, options, cb) {
  ...
  var stmt = new ParameterizedSQL(
    'SELECT COUNT(*) as cnt FROM ' + self.tableEscaped(model)
  );
  stmt = stmt.merge(self.buildWhere(model, where));
  stmt = self.parameterize(stmt);
  self.execute(stmt.sql, stmt.params, function(err, res) {
    if (err) {
      return cb(err);
    }
...
Informix.prototype.buildLimit = function(limit, offset) {
  debug('Informix.prototype.buildLimit limit: %j offset: %j', limit, offset);
...
Informix.prototype.applyPagination = function(model, stmt, filter) {
  ...
  var limitClause = this.buildLimit(filter.limit, filter.offset || filter.skip);
  return stmt + limitClause;
};

Informix.prototype.getCountForAffectedRows = function(model, info) {
  var affectedRows =
    info && typeof info.affectedRows === 'number' ?
      info.affectedRows :
      undefined;
  return affectedRows;
};
...
Informix.prototype.buildColumnDefinition = function(model, prop) {
  ...
  var line =
    this.columnDataType(model, prop) +
    ' ' +
...
Informix.prototype.convertTextType = function convertTextType(p, defaultType) {
  var self = this;
  var dt = defaultType;
  var len = p.length || (p.type !== String ? 4096 : p.id ? 255 : 255);
...
Informix.prototype.buildInsertDefaultValues = function(model) {
  debug('Informix.prototype.buildInsertDefaultValues');
 ...
 */
Informix.prototype.fromRow = Informix.prototype.fromDatabase = function(
  model,
  rowData
) {
  if (rowData == null) {
  ...
  for (var p in props) {
    ...
    if (p === columnName) {
      columnValue = this.fromColumnValue(
        props[p],
 ...
 * @param {Function} [callback] The callback function
 */
Informix.prototype.updateOrCreate = Informix.prototype.save = function(
  model,
  data,
  options,
  callback
  ...
  debug(
    'Informix.prototype.updateOrCreate (enter): model=%j, data=%j, ' +
      'options=%j ',
    model,
    data,
    options
  ...
  function executeWithConnection(connection, cb) {
    ...
    connection.query(countStmt, function(err, countData) {
      debug(
        'Informix.prototype.updateOrCreate (data): err=%j, ' + 'countData=%j\n',
        err,
        countData
      );
      ...
      if (countData[0]['cnt'] > 0) {
        ...
        stmt = self.buildInsert(model, data, options);
      }
      ...
      connection.query(stmt, function(err, sData) {
        debug(
          'Informix.prototype.updateOrCreate (data): err=%j, sData=%j\n',
          err,
          sData
        );
  ...
  if (options.transaction) {
    executeWithConnection(options.transaction.connection, function(
      err,
      data,
      meta
    ) {
