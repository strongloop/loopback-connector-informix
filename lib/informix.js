...
Informix.prototype.destroyAll = Informix.prototype.deleteAll = function(
  ...
  self.execute(selectStmt.sql, selectStmt.params, options,
    function(err, s_info) {
    if (err) {
      callback(err);
    } else {
      if (s_info.length > 0) {
        self.execute(deleteStmt.sql, deleteStmt.params, options,
          function(err, d_info) {
          if (err) {
            return (callback(err));
          }
          return callback(err, {count: s_info[0][id]});
        });
      } else {
...
Informix.prototype.create = function(model, data, options, callback) {
  ...
  self.execute(stmt.sql, stmt.params, options, function(err, info) {
    if (err) {
      callback(err);
    } else {
      var tableName = self.tableEscaped(model);
      var s_stmt = 'SELECT MAX(' + id + ') as id FROM ' + tableName;

      // options.isSync = false;
      self.execute(s_stmt, null, options, function(err, s_info) {
        if (err) {
          return (callback(err));
        }

        callback(err, s_info[0][id]);
      });
    }
  });
};
...
Informix.prototype.update = function(
  ...
  self.execute(selectStmt.sql, selectStmt.params, options,
    ...
    if (err) {
      ...
      if (s_info.length > 0) {
        self.execute(updateStmt.sql, updateStmt.params, options,
          function(err, d_info) {
          if (err) {
            return (callback(err));
          }
...
Informix.prototype.updateOrCreate = Informix.prototype.save =
  function(model, data, options, callback) {
    // This could be implemented using MERGE but haven't worked out how yet.
    // It could also be implemented possibly with synchronous calls or
    // transactions both of which I have not yet attempted.
    callback (Error('Function not implemented'));
    // var self = this;
    // var fields = self.buildFields(model, data);
    // var id = self.idName(model);
    // var sql = new ParameterizedSQL('\nMERGE INTO ' +
    ...
    //   sql.merge('USING ( SELECT * FROM ' + self.tableEscaped(model));
    //   sql.merge('AS VT');
