...
Informix.prototype.destroyAll = Informix.prototype.deleteAll = function(
  ...
  var dStmt = self.buildDelete(model, where, options);
  var sStmt = new ParameterizedSQL('SELECT COUNT(' + id + ')');
  ...
  process.nextTick(function() {
    ...
    sStmt.merge('as cnt FROM ' + tableName);
    sStmt.merge(self.buildWhere(model, where));

    try {
      sData = connection.querySync(sStmt.sql, sStmt.params);

      dData = connection.querySync(dStmt, null);
    ...
    callback(err, {count: sData[0]['cnt']});
  });
};
...
Informix.prototype.updateOrCreate = Informix.prototype.save =
  function(model, data, options, callback) {
    var self = this;
    var id = self.idName(model);
    var stmt, cInfo, connection, currentTx;
    var tableName = self.tableEscaped(model);
    var insertColumns = [];
    var idData = {};
    var isInsert = false;
    ...
    process.nextTick(function() {
      ...
      try {
        var cStmt = new ParameterizedSQL('SELECT COUNT(*) AS cnt FROM ');
        cStmt.merge(tableName);
        cStmt.merge(self.buildWhere(data));

        cInfo = connection.querySync(cStmt.sql, cStmt.params);
