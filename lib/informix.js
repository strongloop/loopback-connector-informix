...
Informix.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  if (options.isSync) {
    var data = conn.querySync(sql, params);
    debug('Informix.prototype.executeSQL (exit)' +
          ' sql=%j params=%j data=%j',
          sql, params, data);
...
Informix.prototype.destroyAll = Informix.prototype.deleteAll = function(
  ...
  var d_stmt = self.buildDelete(model, where, options);
  var s_stmt = new ParameterizedSQL('SELECT COUNT(' + id + ')');

  s_stmt.merge('as cnt FROM ' + tableName);
  s_stmt.merge(self.buildWhere(model, where));

  self.client.open(self.connStr, function(err, connection) {
    if (!connection) {
      return cb(Error('Failed to open connection'));
    }

    var currentTx = connection.beginTransactionSync();

    try {
      s_data = connection.querySync(s_stmt.sql, s_stmt.params);

      d_data = connection.querySync(d_stmt, null);

      connection.commitTransactionSync();
      connection.closeSync();
      return callback(err, {count: s_data[0]['cnt']});
    } catch (e) {
...
Informix.prototype.create = function(model, data, options, callback) {
  ...
  self.client.open(self.connStr, function(err, connection) {
    ...
    try {
      insertData = connection.querySync(insertStmt.sql, insertStmt.params);
      selectData = connection.querySync(selectStmt, null);
      connection.commitTransactionSync();
      connection.closeSync();
      return callback(null, selectData[0][id]);
    } catch (e) {
      connection.rollbackTransactionSync();
      connection.closeSync();
      return callback(e);
...
Informix.prototype.update = function(
  ...
  self.client.open(self.connStr, function(err, connection) {
    ...
    try {
      ...
      connection.commitTransactionSync();
      connection.closeSync();
      return callback(err, {count: selectData[0]['cnt']});
    } catch (e) {
      connection.closeSync();
      return callback(e);
    }
...
Informix.prototype.updateOrCreate = Informix.prototype.save =
  function(model, data, options, callback) {
    ...
    self.client.open(self.connStr, function(err, connection) {
      ...
      try {
        var c_stmt = new ParameterizedSQL('SELECT COUNT(*) AS cnt FROM ');
        c_stmt.merge(tableName);
        c_stmt.merge(self.buildWhere(data));

        c_info = connection.querySync(c_stmt.sql, c_stmt.params);
