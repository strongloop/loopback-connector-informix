...
module.exports = function(Informix) {
  ...
  Informix.prototype.getTableStatus = function(model, cb) {
    debug('Informix.prototype.getTableStatus');
    var self = this;
    ...
    self.execute(sql, function(err, tableInfo) {
      ...
      if (err) {
        ...
        var indexSQL = 'SELECT I.IDXNAME, T.TABNAME, I.IDXTYPE, ' +
  ...
  Informix.prototype.alterTable = function(model, actualFields, actualIndexes,
    ...
    aiNames.forEach(function(indexName) {
      if (ai[indexName].info.idxtype === 'U' ||
    ...
    propNames.forEach(function(propName) {
      ...
      if (!found) {
        ...
        statement.merge(self.tableEscaled(model));
