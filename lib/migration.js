...
module.exports = function(Informix) {
  ...
  Informix.prototype.getTableStatus = function(model, cb) {
    var self = this;
    // var sql = 'SELECT C.COLNAME AS NAME, C.COLTYPE AS DATATYPE, C.TABID, ' +
    // 'C.COLNO, C.COLLENGTH AS C.DATALENGTH FROM SYSCOLUMNS C INNER JOIN ' +
    // '(SELECT TABID FROM SYSTABLES WHERE TABNAME LIKE LOWER(\'' +
    // self.tableEscaped(model) + '\')) VT(x) ' +
    // ' ON T.TABID = x.TABID'; // ORDER BY COLNO';
    ...
    var sql = 'SELECT COLNO, COLLENGTH AS DATALENGTH, COLTYPE AS DATATYPE, ' +
      ...
      'SYSTABLES.TABNAME LIKE LOWER(\'' + self.tableEscaped(model) + '\')' +
      'ORDER BY COLNO';
  ...
  Informix.prototype.alterTable = function(model, actualFields, actualIndexes,
                                      callback, checkOnly) {
    debug('Informix.prototype.alterTable %j %j %j %j %j',
